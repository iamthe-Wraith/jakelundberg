<script lang="ts">
	...
	const onReadMoreToggle = (projectId: number) => {
		return () => {
					...
					{#each data.jobs.data as job, i (job.id)}
						<article class="{(i % 2) ? 'reverse' : ''}">
							...
							<div class="content">
								...
								<div class="metadata">
									<p>
										{dayjs(job.startDate).local().format('MMM, YYYY')} - {job.endDate ? dayjs(job.endDate).local().format('MMM, YYYY') : 'Present'}
									</p>
								</div>
									...
									<div>
										...
										<div class="read-more-toggle">
											<input
												...
												on:change={onReadMoreToggle(project.id)}
											/>
